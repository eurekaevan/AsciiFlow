...
The built-in Stage 0 font creates an 8x8 R8 alpha atlas once per backend and
charset. The renderer composites that atlas directly into a new NV12 frame.
...
legacy code. Full FreeType font loading is deferred; `--font` currently accepts
only `builtin-8x8`.
...
floating-point S-curve in GLSL. The font crate generates the 8x8 R8 atlas; the
